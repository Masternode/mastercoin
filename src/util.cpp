...
const char * const BITCOIN_CONF_FILENAME = "mastercoin.conf";
const char * const BITCOIN_PID_FILENAME = "mastercoind.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "mastercoin" is a composite category enabling all Mastercoin-related debug output
                if(ptrCategory->count(std::string("mastercoin"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\MastercoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\MastercoinCore
    // Mac: ~/Library/Application Support/MastercoinCore
    // Unix: ~/.mastercoincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MastercoinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/MastercoinCore";
#else
    // Unix
    return pathRet / ".mastercoincore";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty mastercoin.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override mastercoin.conf
